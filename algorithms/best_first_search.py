...
class Position():
  def __init__(self, steps, position, path):
    self.steps = steps
...
def best_first_search(grid, start, end, lines, columns):
  ...
  path = [start]
  steps = 0
  pq.put((steps, (start, [start])))
  visited = set()

  while(not pq.empty()):
    steps, (node, path) = pq.get()
    i = node[0]
    j = node[1]
...
def main():
  ...
  maze = readMaze("maze0.txt")
  print(maze.grid)
  path = best_first_search(maze.grid, (maze.startI, maze.startJ), (maze.endI, maze.endJ), maze.lines, maze.columns)
  print("Path:", path)
